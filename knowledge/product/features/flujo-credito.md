...
- **Sprint 3 bloqueado multifrente (Daily 2026-06-03):** el desbloqueo de las plantillas de documentos NO destraba el sprint por sí solo. Se suman bloqueantes de (a) **arquitectura interna** — relación de tablas (qué guardar/qué no) y tabla de cuotas (PER-42, frenada por Nico), decisiones de Quarks aún abiertas; y (b) **infraestructura** — entorno dev/stage + bucket S3 + cadena de conexión (PERC), sin lo cual no se pueden persistir ni testear documentos firmados. El dev solo puede avanzar andamiaje UI. (observation, [ingestion/meetings/2026-06-03-daily-perc.md](../../../ingestion/meetings/2026-06-03-daily-perc.md))
...
| Entorno dev/stage (PERC) | Infraestructura | **Bloqueante Sprint 3** — necesario para persistir firma digital (fecha, IP) y testear. (Daily 2026-06-03) |
| Bucket S3 + cadena de conexión (PERC) | Infraestructura | **Bloqueante Sprint 3** — hay collection de Insomnia pero falta bucket, connection string y stage. (Daily 2026-06-03) |
| Arquitectura de datos interna (Quarks) | Definición técnica | **Bloqueante Sprint 3** — relación de tablas + tabla de cuotas (PER-42, frenada por Nico). (Daily 2026-06-03) |
...
- ~~¿Los documentos HTML son dinámicos o estáticos?~~ **Resuelto 2026-06-01 — dinámicos, formato HTML.** Sebastián confirmó en el design review (1/6): los documentos incluyen variables del usuario/préstamo y van en HTML (no PDF) para permitir inyección de variables + firma embebida. Decisión: [decisions/2026-06-01-documentos-dinamicos-html.md](../../../decisions/2026-06-01-documentos-dinamicos-html.md). **Sigue abierto (distinto):** cuáles documentos exactos son dinámicos vs. estáticos, su tamaño/tipo y restricciones HTML — ver bloqueante de plantillas abajo. (stakeholder-verbal, Sebastián, 2026-06-01)
...
- **Arquitectura: relación de tablas (Sprint 3 — bloqueante).** Cómo plantear las tablas y qué se guarda / qué no. Definición interna de Quarks aún abierta; bloquea tareas de código del dev. (observation, [ingestion/meetings/2026-06-03-daily-perc.md](../../../ingestion/meetings/2026-06-03-daily-perc.md))
- **PER-42 — tabla / integración de cuotas (Sprint 3 — bloqueante).** Frenada por Nico hasta definir la tabla de cuotas. La visualización del préstamo concedido (vista de cuotas) depende de definirla primero. (observation, [ingestion/meetings/2026-06-03-daily-perc.md](../../../ingestion/meetings/2026-06-03-daily-perc.md))
- **PER-54 — implementación de firmas (Sprint 3 — bloqueante).** Cómo encarar técnicamente la firma (qué/cómo/dónde se guarda). Nico espera respuesta de PERC. Open question del PM: confirmar qué parte es decisión interna vs. genuinamente bloqueada por PERC. (observation, [ingestion/meetings/2026-06-03-daily-perc.md](../../../ingestion/meetings/2026-06-03-daily-perc.md))
- **Infra: bucket S3 + cadena de conexión + entorno dev/stage (Sprint 3 — bloqueante).** Hay collection de Insomnia para conectarse a S3, pero falta bucket, connection string y stage; persistir la firma (fecha/IP) requiere entorno dev. Sin esto no se guardan ni testean documentos firmados. (observation, [ingestion/meetings/2026-06-03-daily-perc.md](../../../ingestion/meetings/2026-06-03-daily-perc.md))
