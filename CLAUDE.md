...
2. **Classify the task type** (see § Task types below), which governs the output shape. Getting this wrong is the most common quality failure: producing a routing-summary when the PM asked for substantive synthesis, or vice versa.
...
PM asks you to draft a decision record. Use the `decisions/_SCHEMA.md` template. Every Evidence row carries a provenance tag. Every Status / Date / Reversal-condition field is present. Output to the PM: the decision file path, a 1-line summary of what was committed, and what remains open for PM sign-off.
...
- If you cannot preserve the source (e.g., it's a live URL with no offline copy), record the URL, the retrieved-at timestamp, and a copy of what you saw at the top of the `source/` file. Never substitute "trust me, this came from somewhere" for a real artifact.
...
- **Exceptions**: write directly only for reading and routing, appending to `ingestion/` (raw or working-memory only), updating `Last touched` / `Last updated` auto-maintained fields, and fixing broken markdown links found during retrieval.
...
Propose and wait. Draft all changes and wait for explicit confirmation before writing to any file (except raw ingestion appends and Last touched / Last updated auto-maintained fields). End every task with: "Apply these changes? (y / edit / no)".

### Maintenance cadence
<!-- From Batch E Q2. Options: weekly /review | on-demand only | both. -->
Daily check-in plus weekly /review plus on-demand. Cadence reflects that progress can happen daily.
