# Skill: prototipador (builds the navigable functional prototype, POST discovery-context)
...
See [`briefings/_ciclo-preventa.md`](../../briefings/_ciclo-preventa.md). Runs **after `discovery-context`**. Its relationship to [`/propuestador`](./propuestador.md) depends on whether `propuestador` ran in multi-path mode (see `propuestador.md § Phase 0`):

- **If `propuestador` already decided a path** (passed its Phase 0 checkpoint) → prototype **that chosen path**, not the raw discovery-context. This is the default since the `propuestador` redesign.
- **If `propuestador` hasn't run yet, or ran in "simple case" mode without comparing paths** → falls back to the usual behavior: prototype directly from the discovery-context (§ Hard gate below).
...
- **Ideal:** the path decided by `propuestador` exists (or, if `propuestador` didn't run in multi-path mode, the client's `discovery-context`) → the MVP screens get prototyped with a solid basis.
...
Same criterion as `/propuestador`. The prototype shows **the product**, never our own kitchen: no internal tensions, no stakeholder reads, no competitors, no amounts, no reused internal assets. If something is internal, it stays in the discovery-context.
...
2. **No manual, but researched web presence.** Derive the palette and style from the client's sites already researched by `briefing-context`/`discovery-context` (brand colors, typeface, visual density of the industry).
...
- **Core screens first.** From the discovery-context's MVP scope, pick the happy path that best demonstrates the value. Don't cover the whole roadmap; that confuses MVP with future.
...
- **A two-sided product → prototype both sides (or state which one is missing).** Many products have **a client app + an internal panel**. If the discovery-context's scope lists both, the prototype should ideally show both (with a switch to toggle between them), or at least explicitly say which one was left pending and why.
...
1. Load the **best context available** (discovery-context if it exists; otherwise briefing-context) + branding (manual / web / standard).
...
- **The discovery-context** of the project (MVP scope from `§ B`, users/roles from `§ A3`), or the briefing-context if the discovery-context doesn't exist yet.
...
✅ If `propuestador` ran in multi-path mode, prototypes the chosen path, not the raw discovery-context without a decision
